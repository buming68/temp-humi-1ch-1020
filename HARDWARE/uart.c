...
uint8 Uart1_SEND_Buffer[SEND_MAX_LEN];		//发送缓冲区
...
{	
	...
	AUXR |= 0x05;     //定时器2时钟为Fosc,即1T,定时器2作为波特率发生器
...
void Uart1() interrupt UART1_VECTOR using 1
{
	...
	if (RI)                     //串行接收到停止位的中间时刻时，该位置1
  {
      RI = 0;                 //清除RI位 （该位必须软件清零）
			...
			//非本机地址数据不接收
			//广播地址0，从站地址，首个字符接收时   0xC1为LORA返回信息
			if(Recv_Cnt == 0 || Uart1_RX_Buffer[0] == BaudAddr[1] || Uart1_RX_Buffer[0] == 0x00 || Uart1_RX_Buffer[0] == 0xC1)	//从机地址,在EEPROM.C定义
			{
